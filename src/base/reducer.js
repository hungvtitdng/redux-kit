...
const selectorOf = ({ selector, name }) => (selector === undefined ? name : selector)

const createBaseReducer = (constants, initialStateOverride = {}, allActions = [], operations = []) => {
  const initialState = {
    ...createInitialState(),
    ...
    produce(state || initialState, (draft) => {
      switch (action.type) {
        ...
        case constants.HANDLE_ERROR:
          ...
          allActions.forEach((actionConfig) => {
            const successSelector = actionConfig.successSelector
              || `${selectorOf(actionConfig) || actionConfig.name}Success`
            draft[successSelector] = false
            if (actionConfig.loadingType) draft[actionConfig.loadingType] = false
          })
