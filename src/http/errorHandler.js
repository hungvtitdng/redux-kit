...
export const startsWithAny = (url = "", prefixes = []) => prefixes.some((prefix) => url.indexOf(prefix) === 0)
...
export const createErrorHandler = ({
  ...
  return (error) => {
    ...
    switch (statusCode) {
      ...
      case 404:
        if (!silentNotFoundPaths.includes(url) && !startsWithAny(url, silentNotFoundSubPaths)) {
      ...
      case 422:
        if (!payload) {
          ...
          notifyValidationErrors(payload.errors || payload.message, notifyError)
        }
        break
