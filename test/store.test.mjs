...
test("CRUD action: endpoint -> api -> saga -> reducer", async () => {
  ...
  const transaction = createBaseStore({
    name: "transaction",
...
test("operation: own loading flag, selector and after hook", async () => {
  ...
  const inventory = createBaseStore({
    ...
    customApiMethods: {
      export: (params) => Promise.resolve({ url: "/tmp.xlsx", params }),
    },
    operations: [
      {
        name: "export",
  ...
  assert.deepEqual(store.getState().inventory.export, {
    url: "/tmp.xlsx",
    params: { ids: [1] },
  });
    ...
    operations: [
      {
        name: "verify",
        selector: null,
        saga: { error: () => seen.push("logged-out") },
      },
      {
        name: "logout",
        selector: null,
        saga: { after: () => seen.push("bye") },
      },
    ],
  });
  mount(store, auth);
...
test("API failure lands in error and clears loading", async () => {
  ...
  const { createBaseStore } = createKit({
    http: fakeHttp({ get: () => Promise.reject(boom) }),
  });
...
test("api object without CRUD: no base actions generated", async () => {
  const optionApi = {
    accounts: (params) => Promise.resolve({ data: ["a"], params }),
  };
  const { createBaseStore } = createKit({ http: fakeHttp() });
    ...
    operations: [
      {
        ...
        payload: ["params"],
        selector: "accounts",
        successSelector: "getAccountsSuccess",
      },
    ],
  });
  mount(store, options);
  ...
  assert.deepEqual(store.getState().options.accounts, {
    data: ["a"],
    params: { q: 1 },
  });
...
test("static reducers survive module injection, no module-level state between stores", () => {
  ...
  const storeA = configureStore({
    staticReducers: { app: (state = { booted: "a" }) => state },
  });
  const storeB = configureStore({
    staticReducers: { app: (state = { booted: "b" }) => state },
  });
...
test("selector: null keeps no response in state", async () => {
  ...
  const contact = createBaseStore({
    ...
    endpoint: "contacts",
    operations: [
      { name: "ping", apiName: "list", payload: ["params"], selector: null },
    ],
  });
  mount(store, contact);
